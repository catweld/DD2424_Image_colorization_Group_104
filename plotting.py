...
def plot_prediction(settings, model, w, image_path, savefig=False, save_image=False, T = 0.0):
    ...
    # Greyscale image
    lab_image_grey = combine_lab_grey(L)
    # Back to rgb for plotting
    grey = lab2rgb(lab_image_grey)
    ax1 = f.add_subplot(144)
    predition = plt.imshow(grey)
    ...
    if savefig:
        ...
        plt.imsave(str(fig_name) + '_grey.png', grey)
...
def combine_lab_grey(L):
    # Combines LAB to image
    img_combined = np.zeros(([L.shape[0], L.shape[1], 3]))
    img_combined[:, :, 0] = L
    return img_combined
...
def colorize_images_in_folder(settings, model, w, folder_path):
    ...
    for image in tqdm(images):
        if 'pred' not in image and 'grey' not in image:
            file_path = folder_path + image
            plot_prediction(settings, model, w, file_path, save_image=True, T=0.38)
